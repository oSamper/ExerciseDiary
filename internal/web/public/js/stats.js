...
function addSet(i, date, reps, weight, series, rate) {
    // Rate icon mapping
    const rateIcons = {
        1: '✗',
        2: '○',
    ...
    const rateIcon = rateIcons[rate] || rate;

    html_code = '<tr><td style="opacity: 45%;">' + i + '.</td><td>' + date + '</td><td>' + reps + '</td><td>' + weight + '</td><td>' + series + '</td><td>' + rateIcon + '</td></tr>';
...
function setStatsPage(sets, hcolor, off, step) {
    let start = 0, end = 0;
    let dates = [], ws = [], reps = [], series = [], exs = [], rates = [];
    ...
    for (let i = start; i < end; i++) {
        addSet(i + 1, exs[i].Date, exs[i].Reps, exs[i].Weight, exs[i].Series, exs[i].Rate);

        dates.push(exs[i].Date);
        reps.push(exs[i].Reps);
        ws.push(exs[i].Weight);
        series.push(exs[i].Series);
        rates.push(exs[i].Rate);
    };

    statsChart('stats-reps', dates, reps, series, hcolor, true);
    weightChart('stats-weight', dates, ws, hcolor, true, rates);
};
